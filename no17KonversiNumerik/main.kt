fun main(){
    val nilai:Int = 40
    val nilaiByte:Byte = nilai.toByte()
    val nilaiString:String = nilai.toString()
    val nilaiDouble:Double = nilai.toDouble()
    val nilaiLong:Long = nilai.toLong()
    val nilaiShort:Short = nilai.toShort()
    val nilaiChar:Char = nilai.toChar()
    //kode desimal 40 karakter ASCII adalah simbol '('

    println("---------------------------------------------------")
    println("Latihan Konversi Numerik : Rizwan Elansyah")

    println("Nilai Int : $nilai")
    println("Nilai Byte : $nilaiByte")
    println("Nilai String : $nilaiString")
    println("Niali Double : $nilaiDouble")
    println("Nilai Long : $nilaiLong")
    println("Nilai Short : $nilaiShort")
    println("Nilai Cahr : $nilaiChar")

    println("---------------------------------------------------")
}